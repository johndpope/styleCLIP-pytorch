...
from styleCLIP import encoder, decoder, visual
...
def GetFs(fs):
    ...
    Args:
        fs ([torch.Tensor]): number of manipulations (layers * comps), num_images, num_alphas, 512

    Returns:
        fs3 ([torch.Tensor]): 1, 512
    """
    tmp=np.linalg.norm(fs,axis=-1) 
    fs1=fs/tmp[:,:,:,None]
    fs2=fs1[:,:,1,:]-fs1[:,:,0,:]  # alpha*sigma - alpha* sigma
        ...
        for layer_idx in range(generator.n_latent):
            for pc_idx, pc in enumerate(comp):
                ...
                for img_idx in range(args.num_images):
                    ...
                    for alpha_idx, alpha in enumerate([20, -20]):
                        ...
                        out[img_idx, alpha_idx, :, :, :] = visual(image, save=False, name=str(i))
            
                shp = list(out.shape[2:]) # 1024, 1024, 3
                tmp = []
                img = out.reshape([-1]+shp)
                ...
                image = torch.cat(tmp)
                with torch.no_grad():
                    image_features = model.encode_image(image) # num alpha * num images, 512
                image_features = image_features.cpu().numpy()
                image_features = image_features.reshape([args.num_images, 2, 512])
                all_features.append(image_features)   
                print(f"Created image feature vector for {layer_idx}-th layer with {pc_idx}-th PC: Total of {len(all_features)}")
